#!/usr/bin/env bash
# Build the bottom-strip Ocean-style loader PRG with 64tass.
...
OUT_D64="$ROOT/build/OCNBOT.D64"
if command -v c1541 >/dev/null 2>&1; then
    rm -f "$OUT_D64"
    c1541 -format "pakito bot,01" d64 "$OUT_D64" >/dev/null
    c1541 -attach "$OUT_D64" -write "$OUT_PRG" "ocnbot" >/dev/null
